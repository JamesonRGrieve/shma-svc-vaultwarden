...
- `vaultwarden_database_max_conns` wires directly into `DATABASE_MAX_CONNS`, expanding the connection pool when external services (Directory Connector, mobile clients) spike concurrency.
...
- The bundled backup hook runs `sqlite3 /data/db.sqlite3 '.backup "/data/db.sqlite3.backup"'`. When WAL is enabled, follow the backup with `sqlite3 /data/db.sqlite3 'PRAGMA wal_checkpoint(TRUNCATE);'` to keep log files small.
...
- Switch to S3-compatible object storage by setting `vaultwarden_attachments_backend: s3` and providing the endpoint credentials:
  ...
  vaultwarden_attachments_s3_access_key: "{{ vaultwarden_s3_access_key }}"
  vaultwarden_attachments_s3_secret_key: "{{ vaultwarden_s3_secret_key }}"
  vaultwarden_attachments_s3_path_style: true  # for MinIO/compatible endpoints
  ```
 
## Monitoring & Operations
- Metrics: expose Vaultwarden counters through [`dani-garcia/vaultwarden_exporter`](https://github.com/dani-garcia/vaultwarden_exporter) or [`ViViDboarder/vaultwarden_exporter`](https://github.com/ViViDboarder/vaultwarden_exporter). Deploy it as a sidecar scraping `/metrics` and register it with Prometheus.
...
| `vaultwarden_database_max_conns`   | ``                          | Connection pool size for PostgreSQL/MySQL backends    |
...
| `vaultwarden_max_attachment_size`  | ``                          | Maximum attachment size in bytes                      |
| `vaultwarden_attachment_storage_quota` | ``                     | Total attachment storage quota                        |
...
2. **Bootstrap Signups**: Temporarily set `vaultwarden_signups_allowed: true` for initial admin onboarding, create the first user, then immediately revert to `false` in inventory and redeploy. Document the window in change control records.
